...
/**
 * Shared, reusable table core for the RM and PM rate views (Vendor Rate +
 * Manufacturer Rate tabs for both material types). RmRateTable.tsx and
 * PmRateTable.tsx are thin config wrappers around this — they were
 * byte-for-byte identical except for a handful of labels/endpoints, which
 * quietly drifted out of sync (the "Make" vs "Make / Type" filter label, the
 * type-filter-writes-a-different-URL-key quirk) since a fix to one copy had
 * no way of reaching the other. This component owns:
 *   - URL-synced search (UrlSearchInput, 350 ms debounce)
 *   - URL-driven filter panel (status/type/make/vendor/mfg/rate-range/effective-from)
 *   - Client-side sort within the current page (click column header)
 *   - PaginationBar footer (prev/next, page-size selector)
 *   - CsvImportDialog + Add-wizard actions
 *
...
type AddWizardProps = { vendors: Vendor[]; manufacturers: Mfg[]; onSuccess: () => void }
...
export function MaterialRateTable({
  // Material-type identity — the only things that differ between RM and PM.
  entityLabel,
  searchPlaceholder,
  downloadEndpoint,
  vrmBulk,
  ...
  AddWizard,
  makeFieldLabel = "Make",
  // PM's Type filter writes to the same `make` URL key as its Make select
  // (they're never both rendered — vendor view shows `makes`, mfg view shows
  // `types`); RM keeps them as separate `type`/`make` keys.
  combineTypeIntoMake = false,
  emptyMessage,
  // Data + pagination/filter state (URL-driven):
  rows,
  columns,
  actionColumn,
  vendors,
  ...
  vrmBulk: BulkImportConfig
  mrmBulk: BulkImportConfig
  AddWizard: ComponentType<AddWizardProps>
  makeFieldLabel?: string
  combineTypeIntoMake?: boolean
  emptyMessage: string
  rows: AnyRow[]
  columns: ColumnDef[]
  ...
  const clearedFilterUrl = { search: "", status: "", type: "", make: "", vendor_code: "", rate_min: "", rate_max: "", effective_from: "", mfg_code: "", mfg_rate_min: "", mfg_rate_max: "", mfg_effective_from: "" }

  // Commits every draft filter value to the URL in one navigation.
  function applyFilters() {
    navigate({
      status: draftStatus,
      ...(combineTypeIntoMake ? { make: draftType || draftMake } : { type: draftType, make: draftMake }),
  ...
  return (
    <>
        ...
        <MasterToolbarActions>
          ...
          {isMfgView ? (
            <CsvImportDialog
              ...
              endpoint={mrmBulk.endpoint}
              templateFilename={mrmBulk.templateFilename}
              fields={mrmBulk.fields}
            ...
            <CsvImportDialog
              ...
              endpoint={vrmBulk.endpoint}
              templateFilename={vrmBulk.templateFilename}
              fields={vrmBulk.fields}
              enableDuplicateCheck
              requireAllValid
              onSuccess={refresh}
            />
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {makes !== undefined && (
                <>
                  <div className="grid gap-1.5">
                    <Label className="text-xs">{makeFieldLabel}</Label>
                    <Select
                      value={draftMake || "all"}
      ...
      <Card>
        <RecordCountHeader
          total={total}
          onClearFilters={hasFilters ? () => navigate(clearedFilterUrl) : undefined}
        />
          ...
          <Table className="[&_th]:whitespace-nowrap table-fixed">
            <TableHeader>
              <TableRow>
                ...
                {actionColumn && <StaticTableHead width="112px">Actions</StaticTableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {sorted.length === 0 ? (
                <TableRow>
                  ...
                  >
                    <EmptyState hasFilters={!!hasFilters} filteredMessage={emptyMessage} />
                  </TableCell>
                </TableRow>
              ) : (
