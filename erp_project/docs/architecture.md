...
| `lib/pdf/po-document.tsx` | React PDF template for PO documents — **ONE** A4 template for every legal entity, used by preview and email send. Everything that differs between Pep and Kreative arrives as data on `d.letterhead` / `d.ship_to`, never as a second template |
| `lib/pdf/po-letterhead.ts` | Resolves that data: `sku_code → master_skus.brand_id → master_brand.entity_id → master_entity` for the legal name and bank block, and `destination → details_warehouse_entity` for the per-(site, entity) bill-to and ship-to. Resolved at **render** time, so attributing a SKU's brand fixes every existing PO's PDF without touching a `purchase_orders` row |
