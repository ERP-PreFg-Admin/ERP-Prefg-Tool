...
export function PmRateHistoryDialog({
  row,
  kind,
  onClose,
}: {
  /** Pass null to close. Must carry pm_id + (mfg_id for kind="mfg" | vendor_id for kind="vendor") + name/code for the title. */
  row: { pm_id: number; mfg_id?: number | null; vendor_id?: number | null; name?: string | null; code?: string | null } | null
  kind: "mfg" | "vendor"
  onClose: () => void
}) {
  ...
  useEffect(() => {
    ...
    const endpoint = kind === "mfg"
      ? `/api/masters/packing-materials/mrm-history?pm_id=${row.pm_id}&mfg_id=${entityId}`
      : `/api/masters/packing-materials/vrm-history?pm_id=${row.pm_id}&vendor_id=${entityId}`
    fetch(endpoint)
      .then((r) => r.json())
