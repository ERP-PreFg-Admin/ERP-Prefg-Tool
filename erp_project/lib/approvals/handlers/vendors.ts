...
/** Everything an approved VENDOR edit may write. Doc keys sit on details_vendor
 *  too, so there is no separate document pass any more. */
const VENDOR_BASE: ColumnTarget = {
  table: "master_vendors", key: "id", columns: ["name", "type"],
}
const VENDOR_DETAILS: ColumnTarget = {
  table: "details_vendor", key: "vendor_id",
  columns: [
    "location", "status", "zone", "registered_name", "gst_number",
    "bank_name", "ifsc_number", "account_number",
...
export const vendorHandler: ModuleHandler = {
  ...
  async applyAndArchive(conn, entityId, items) {
    ...
    if (!(rows as unknown[])[0]) throw new Error(`Vendor ${entityId} not found`)

    await applyApprovedColumns(conn, VENDOR_BASE, fieldMap, entityId)
    ...
    await applyApprovedColumns(conn, VENDOR_DETAILS, fieldMap, entityId, {
