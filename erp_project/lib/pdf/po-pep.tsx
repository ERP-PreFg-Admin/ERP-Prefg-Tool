/**
 * The Pep Technologies purchase order. Also the document any PO whose brand
 * resolves to no legal entity gets — see the default arm in po-document.tsx.
 *
 * Layout unchanged from the single-template version; what changed is that the
 * letterhead, the delivery address and the bank block are now DATA
 * (lib/pdf/po-letterhead.ts) instead of the hardcoded `COMPANY` constant that
 * printed Pep's name on every Kreative PO too.
 *
 * This file renders. It resolves nothing and has no fallbacks of its own — that
 * ladder lives in po-letterhead.ts so that po-kreative.tsx cannot disagree with it.
 */

import React from "react"
import { Document, Page, Text, View, StyleSheet } from "@react-pdf/renderer"
import { type PoEmailData, fmtN, fmtDate, num, poTotals, GST_PERCENT_LABEL } from "@/lib/pdf/po-shared"
...
export function PepPoDocument({ d }: { d: PoEmailData }) {
  const { base, gst, grand } = poTotals(d)
    ...
    <Document>
      <Page size="A4" style={S.page}>
        ...
        <View style={S.infoWrap}>
          ...
          {/* Billing Address — the entity's registration for this destination */}
          ...
          {/* Delivery Address — the consignee, whose GSTIN may be another
              entity's; see resolveShipTo */}
          <View style={S.infoCol}>
        ...
        <View style={S.btm}>
          ...
          <View style={S.totalHL}>
            ...
            <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 8, marginRight: 20 }}>{GST_PERCENT_LABEL}</Text>
            <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 9, color: TEAL }}>
              {grand > 0 ? fmtN(grand) : "—"}
            </Text>
          </View>

          {/* Company Bank Details — the heading alone when the entity has no bank
              on file, which is how this band shipped for months. resolveBank
              returns null unless name, A/C and IFSC are all present, so a
              half-filled block is never printed. */}
          <View style={S.bankHL}>
